...
const DEFAULT_TEXT = "Software Architect";
...
export const StrokeText: React.FC<StrokeTextProps> = ({
  ...
  const numFontSize = typeof fontSize === "number" ? fontSize : parseFloat(fontSize) || 42;
  const dash = Math.max(numFontSize * 7, 200);

  const fontStyle: React.CSSProperties = useMemo(
    () => ({
